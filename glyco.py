...
# GLYCO
#
# 07/27/2021 and updated 10/24/2021
...
def outermost(inpdb, infloat, freesasa_path, inprobe, out_folder):
    ...
    outrsa = out_folder + "/" + Path(inpdb).name.replace(".pdb", "") + "_all_outer.rsa"
...
def merge_dict_lists(added_dict=None, inplace_dict=None, val_unique=True):    
    for k, v in added_dict.items(): 
        if k in inplace_dict.keys(): 
            if val_unique:
                inplace_dict[k] = list(numpy.unique(inplace_dict[k] + v))
            else:
                inplace_dict[k] += v 
        else: 
            inplace_dict[k] = v 
...
def process_cmd_args():
    ...
    parser = argparse.ArgumentParser(usage= '\n'+ '\n' +
             ...
             'module "sub": ' + '\n'
             'python3 glyco.py -in_folder input -cutoff 26  -module sub -glycan BMA,AMA,BGL -residue subset_residue.txt -num_proc_in 22 -num_parallel 2 -out_folder results -average' + '\n'
             '----------------------------------------------------------------------------------------------------------------------------------------------------' + '\n' + '\n')
...
def launch_workers(gly_dict, res_dict, pro_dict, nproc, dist_cutoff, ori_pro_dict, out_folder):
    ...
    for job_id, outter_data in enumerate(bundle_outter_data): 
        ...
        # Add worker to queue
        res = tp.apply_async(worker, (job_id, protein_mini, glycan_mini, pro_dict, dist_cutoff, ori_pro_dict, out_folder)) 
...
def worker(job_id, res_dict, gly_dict, pro_dict, dist_cutoff, ori_pro_dict, out_folder):
...
def aggregate_worker_res(all_results):
    ...
    for final_dict_i in all_results.values():
        ...
        # merge_dict_lists(added_dict=count_dict_i, inplace_dict=count_dict, val_unique=True)
        # merge_dict_lists(added_dict=num_dict_i, inplace_dict=num_dict, val_unique=True)
...
def write_outputs(all_or_sub, final_dict, out_folder, prefix, origin_pdb):
    ...
    if all_or_sub == 'all':
        ...
        #revision
        print(out_folder + "/" + prefix + '_all_glysum.txt')
        with open(out_folder + "/" + prefix + '_all_glysum.txt', 'w') as f_obj_res_sum: 
            total_glycan_list = []
            for key, val in final_dict.items():
                total_glycan_list.append(int(len(val)))
            f_obj_res_sum.write(str(sum(total_glycan_list)))

        print(origin_pdb)
        print(out_folder + "/{}_all_bfactor.pdb".format(prefix))
          
        os.system("cp " + origin_pdb + ' ' + out_folder + "/" + prefix + '_all_bfactor.pdb')
        for line in fileinput.input(out_folder + "/" + prefix + '_all_bfactor.pdb', inplace=1):
            line = line.strip()
        ...
        with open(out_folder + "/" + prefix + '_sub_glysum.txt', 'w') as f_obj_sub:        
            total_glycan_list = []
            for key, val in final_dict.items():
                total_glycan_list.extend(val)
            total_glycan = list(OrderedDict.fromkeys(total_glycan_list))
            f_obj_sub.write(str(len(total_glycan)))
...
def process_request(args, origin_pdb):
    ...
    # Launch workers
    all_results = launch_workers(gly_dict, res_dict, pro_dict, nproc_in, dist_cutoff, ori_pro_dict, out_folder)

    # Retrieve and aggregate data from all workers
    final_dict = aggregate_worker_res(all_results)
    

    # Write results
    prefix = Path(origin_pdb).name.replace(".pdb", "")
    write_outputs(all_or_sub, final_dict, out_folder, prefix, origin_pdb)
...
def average_frames_res(out_folder):
    ...
    #revision
    output_files = list(glob.glob(os.path.join(out_folder, "*all_glysum.txt")))
    glysum = 0
    total_average = 0
    ...
    with open(out_folder + '/ave_all_glysum.txt', 'w') as file_obj:
        file_obj.write(str("{:>9.2f}".format(total_average)) + '\n')
    

def average_frames_sub(out_folder):
    ...
    with open(out_folder + '/ave_sub_glysum.txt', 'w') as file_obj:
        file_obj.write(str("{:>9.2f}".format(total_average)) + '\n')
