...
def move_to_orig(inpdb, outpdb, intcl):
    os.system("vmd -dispdev text -e " + str(intcl) + " -args " + \
               str(inpdb) + " " + str(outpdb))
    return outpdb

def outermost(inpdb, infloat):
    #print('PROGRESS: Excluding buried residues in the protein')
    outrsa = 'outer.rsa'
...
def gen_gly_dict(inpdb):
    outdict = {}
    for line in open(inpdb):
        if line[0:4] == 'ATOM' and line[17:21].strip() in 'BGL, BMA, AMA':
                resname = line[17:21].strip()
                chain = line[21].strip()
                resid = line[22:28].strip()
                atomnum = line[6:11].strip()
...
input_pdb = sys.argv[4]

tcl_file_move = '/data/SBIS/leem25/glycan_density/hiv_man9_n160k/move_to_origin.tcl' 
move_to_orig(input_pdb, origin_pdb, tcl_file_move)
